...
export function Portfolio() {
  ...
  return <>
    ...
    <main id="conteudo">
      ...
      <div className="container stats" aria-label="Números de destaque">{[[String(projects.length), "Projetos"], ["100k+", "Seguidores"], ["3", "Formações acadêmicas"], ["10", "Certificações"]].map(([number, label]) => <div key={label}><strong>{number}</strong><span>{label}</span></div>)}</div></section>
